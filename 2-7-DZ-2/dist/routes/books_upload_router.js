"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const express_1 = __importDefault(require("express"));
const multer_1 = __importDefault(require("multer"));
const add_book_from_file = require('../other_external_functions/add_book_from_file');
const books_upload_router = express_1.default.Router();
const path = require('path');
////конфигурирую storage
const storageConfig = multer_1.default.diskStorage({
    destination: (req, file, cb) => {
        cb(null, 'uploads');
    },
    filename: (req, file, cb) => {
        cb(null, Date.now() + '-' + file.originalname);
    },
});
books_upload_router.use((0, multer_1.default)({ storage: storageConfig }).single('filedata'));
function confirmation(req, res) {
    let filedata = req.file;
    if (!filedata)
        res.send('Ошибка при загрузке файла');
    else {
        res.send('Файл загружен');
        let fileName = path.join(`${path.resolve('./uploads')}/${req.file.filename}`);
        setTimeout(() => {
            add_book_from_file(fileName);
        }, 0);
    }
}
/// ниже сам роутинг
books_upload_router.post('/upload', confirmation); //
exports.default = books_upload_router;
